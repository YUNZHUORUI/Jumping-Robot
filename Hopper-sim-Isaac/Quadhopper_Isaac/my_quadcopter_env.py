...
# 注意：请确保你的目录结构中存在 my_drone_cfg.py
...
class QuadcopterEnvCfg(DirectRLEnvCfg):
    ...
    action_space = 4
    observation_space = 25  # 3(vel) + 3(ang_vel) + 4(quat) + 3(pos_err) + 12(history)[cite: 1]
    state_space = 0
    debug_vis = True
    ...
    yaw_error_reward_scale = -5
    yaw_vel_reward_scale = -1

    action_rate_reward_scale = -5.0  # 原来是 -1.2 (惩罚前后两帧的动作差)
    action_smoothness_reward_scale = -3.0  # 原来是 -0.8 (惩罚动作曲线的二阶导，即毛刺)
...
class QuadcopterEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: QuadcopterEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.dr_mass_multi = torch.ones(self.num_envs, 1, device=self.device)  # 模拟质量变化
        self.dr_inertia_multi = torch.ones(self.num_envs, 3, device=self.device)  # 模拟三轴惯量变化
        self.dr_tm = torch.ones(self.num_envs, 4, device=self.device) * 0.0690
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # 1. 模拟 10ms 控制延迟[cite: 1]
        delayed_actions = self._action_history[-3]

        if len(self._action_history) > 2:
            delayed_actions = self._action_history[-3]  # 强行拉大延迟
        else:
        ...
        # 应用推力增益及质量随机化 (F_eff = F / mass_multi)
        ...
        # 应用力矩增益及惯量随机化 (Tau_eff = Tau / inertia_multi)
        ...
        tau_z = K_tau * -(u1 ** 2 + u3 ** 2 - u2 ** 2 - u4 ** 2) * self.dr_torque_multi.squeeze() / self.dr_inertia_multi[:,
                                                                                              2]
    ...
    def _get_observations(self) -> dict:
        ...
        # 【添加传感器噪音】[cite: 1]
        pos_error += torch.randn_like(pos_error) * 0.01  # 1cm 位置误差
        quat = torch.nn.functional.normalize(quat + torch.randn_like(quat) * 0.005, p=2, dim=-1)  # 姿态噪声
        lin_vel += torch.randn_like(lin_vel) * 0.02  # 线速度噪声
        ang_vel += torch.randn_like(ang_vel) * 0.01  # 角速度噪声

        # 观测屏蔽机制 (Masking)[cite: 1]
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # 【域随机化重置】[cite: 1]
        self.dr_thrust_multi[env_ids] = torch.empty(num_resets, 4, device=self.device).uniform_(0.85,1.15)  # 扩大推力误差到 ±15%
        self.dr_torque_multi[env_ids] = torch.empty(num_resets, 1, device=self.device).uniform_(0.85, 1.15)
        # 【关键修改】：电机时间常数 tm。让电机变得更不可预测，更“肉”
        self.dr_tm[env_ids] = torch.empty(num_resets, 4, device=self.device).uniform_(0.04, 0.12)  # 原来是 0.06~0.08
